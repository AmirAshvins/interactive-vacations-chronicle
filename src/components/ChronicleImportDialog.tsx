...
interface ChronicleImportDialogProps {
  ...
  onClose: () => void;
  onImported: (trips: ImportTrip[], label: string) => void;
}
...
export default function ChronicleImportDialog({
  ...
  const confirmReplace = useCallback(
    (count: number, label: string) =>
      !hasExistingTrips ||
      window.confirm(
        `Import ${count} journal${count === 1 ? '' : 's'} from ${label}? This will replace your current chronicle.`,
      ),
    [hasExistingTrips],
  );
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        if (!confirmReplace(imported.length, 'file')) return;
        finishImport(imported, 'file');
      } catch (err) {
        setStatus({
          type: 'error',
  ...
  const handleApplyCode = useCallback(async () => {
    ...
    try {
      ...
      if (!confirmReplace(imported.length, template.title)) return;
      finishImport(imported, template.code);
    } catch (err) {
      setStatus({
        type: 'error',
        message: err instanceof Error ? err.message : 'Import failed.',
  ...
  }, [code, confirmReplace, finishImport]);
